...
export function buildActiveCartContextMessage(cartId) {
  ...
  return {
    role: "system",
    content:
      "This conversation has an active cart. Use add_to_cart (one product via variant_id, or multiple via items[] in ONE call), " +
      "remove_from_cart (one product), update_cart_items (qty/remove for multiple cart lines), clear_my_cart (remove all), get_my_cart, " +
